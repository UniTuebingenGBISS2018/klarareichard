...
public class HMM{
    ...
    public String runViterbi(String seq) throws Exception {
        ...
        for (int i = 1; i <= L; i++) {
            ...
            for (int l = 0; l < numStates; l++) {
                ...
                for(int k = 0; k < numStates; ++k){
                    double entry = v[k][i-1] * transMatrix[k][l];
                    if(entry > max){
                        max = entry;
                        argMax = k;
                    }else if(entry == max){
                    ...
                    //maxArray.add(v[k][i-1] * transMatrix[k][l]);
                }
                v[l][i] = max * emissionProb[l][xi] ;
                ptr[i][l] = argMax;
                if(argMax == -1){
                    throw new Exception("argmax is -1, something is wrong");
                }
                //v[l][i] =
            }
        }
        ...
        for (int k = 0; k < numStates; k++) {
            if(v[k][L] >= maxValue){
                maxValue = v[k][L] * transMatrix[k][0];
                maxPtr = ptr[L][k];
            }
            // todo: Please implement
        }
